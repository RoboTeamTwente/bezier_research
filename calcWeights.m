...
%     center_weights(ID) = cos(angleToNode/2)^2; % if angle is 0, result is 1. if angle is pi, result is 0
% end
...
for i = 1:length(startPolygon)
    ...
    if ~isempty(ind)
        ...
        center_weights(ind) = sin(angleToNode/2)^2; % if angle is 0, result is 0. if angle is pi, result is 1
    end
end

% Set the weight of the end node to a low value, so that it goes to the end
% point when the angle is quite okay
%center_weights(end) = 0.4;
end