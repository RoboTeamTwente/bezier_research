function [path] = findShortestPath(nodesIN, segments, startID, endID, seg_weights)
...
for i = 1:numNodes
    ...
    nodes(j) = struct('ID',j,'x',nodesIN(ind,2),'y',nodesIN(ind,3),'via',NaN,'dist_start',inf,'dist_end',dist_to_end,'visited',false,'cost',[]);
...
while queue(1) ~= endID % while the end node is not on top of the queue
    ...
    childSegments = [find(segments(:,2)==parentID); find(segments(:,3)==parentID)]; % indices of segments that connect children to their parent
    for i = 1:length(children)
        childID = children(i); % ID of node on the other side of the segment
        currentSegment = childSegments(i); % index of segment that connects child to parent
        if ~nodes(childID).visited
            ...
            % Calculate cost
            childCost = (dist_to_start + nodes(childID).dist_end) * seg_weights(currentSegment);
            if isempty(nodes(childID).cost) || nodes(childID).cost > childCost
                nodes(childID).cost = childCost;
                % update node info
            ...
            if isempty(queue)
                ...
                for j = 1:length(queue)                    
                    if nodes(childID).cost < nodes(queue(j)).cost
                        if j == 1
                            queue = [childID, queue];
                        else
